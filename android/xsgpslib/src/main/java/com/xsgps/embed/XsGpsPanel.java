...
public final class XsGpsPanel extends Dialog {
    ...
    public XsGpsPanel(Activity host){
        super(host, android.R.style.Theme_DeviceDefault_NoActionBar);
        this.activity=host;
        build();
        updateStatus();
        XsGpsLicense.installId(activity);
        if(XsGpsLicense.prefs(activity).getBoolean("license_active",false))
            XsGpsLicense.verify(activity,(ok,msg)->{if(licenseState!=null)licenseState.setText("التفعيل: "+msg);});
        setOnDismissListener(d->bg.shutdownNow());
    }
    ...
    private int dp(int n){return (int)(n*activity.getResources().getDisplayMetrics().density+0.5f);}
    private LinearLayout vertical(){LinearLayout l=new LinearLayout(activity);l.setOrientation(LinearLayout.VERTICAL);return l;}
    private TextView label(String text,int size,int color,boolean bold){
        TextView t=new TextView(activity);t.setText(text);t.setTextSize(size);t.setTextColor(color);
    ...
    private EditText field(String hint,String initial,boolean numeric){
        EditText e=new EditText(activity);e.setSingleLine(true);e.setTextColor(WHITE);
    ...
    private Button btn(String text,boolean primary,Runnable action){
        Button b=new Button(activity);b.setText(text);b.setAllCaps(false);b.setTextSize(15);
    ...
    private void build(){
        ...
        pad(root,16,24,30);
        ScrollView sc=new ScrollView(activity);sc.setFillViewport(true);sc.addView(root);
        setContentView(sc);
        TextView brand=label("XsGPS  •  ANDROID",28,GREEN,true);
        root.addView(brand);
        addButton(root,"إغلاق لوحة XsGPS",false,this::dismiss);
        TextView subtitle=label("لوحة مدمجة في التطبيق — اختبار الموقع الرسمي بدون روت",13,MUTED,false);
        ...
        latitude=field("خط العرض Latitude",
                XsGpsLicense.prefs(activity).getString("lat","24.7136"),true);
        longitude=field("خط الطول Longitude",
                XsGpsLicense.prefs(activity).getString("lon","46.6753"),true);
        ...
        randomMovement=new CheckBox(activity);randomMovement.setText("تفعيل الحركة العشوائية البطيئة");
        randomMovement.setTextColor(WHITE);
        randomMovement.setChecked(XsGpsLicense.prefs(activity).getBoolean("movement",false));
        motion.addView(randomMovement);
        radiusLabel=label("نطاق الحركة: 20 مترًا",15,MUTED,false);
        motion.addView(radiusLabel);
        radiusSeek=new SeekBar(activity);radiusSeek.setMax(100);
        radiusSeek.setProgress(XsGpsLicense.prefs(activity).getInt("radius",20));
        ...
        addButton(bookmarks,"حفظ الإحداثيات الحالية",false,()->{
            try{
                ...
                arr.put(item);
                XsGpsLicense.prefs(activity).edit().putString("bookmarks",arr.toString()).apply();
        ...
        licenseState=label(!XsGpsOverlay.activationRequired()?
        ...
        code=field("كود Android (10 خانات)",
                XsGpsLicense.prefs(activity).getString("license_code",""),false);
        ...
        addButton(activation,"تفعيل كود Android",true,()->{
            ...
            XsGpsLicense.activate(activity,value,(ok,msg)->{
                licenseState.setText("التفعيل: "+msg);
                toast(msg);
            });
        });
        TextView footer=label("تطبيق الموقع التجريبي يتطلب اختيار التطبيق المستضيف نفسه في خيارات المطوّر. لا يخفي هذا الموقع الوهمي عن التطبيقات.",12,MUTED,false);
        root.addView(footer);
    }
    ...
    private void savePosition(double lat,double lon){
        ...
        XsGpsLicense.prefs(activity).edit().putString("lat",latitude.getText().toString())
                .putString("lon",longitude.getText().toString()).apply();
    }
    private JSONArray saved(){
        try{return new JSONArray(XsGpsLicense.prefs(activity).getString("bookmarks","[]"));}
    ...
    private void refreshSaved(){
        ...
        for(int i=0;i<arr.length();i++){
            ...
            b.setOnLongClickListener(v->{
                new AlertDialog.Builder(activity).setTitle("حذف الموقع؟").setMessage(title)
                    .setPositiveButton("حذف",(d,w)->{
                        JSONArray source=saved(),next=new JSONArray();
                        for(int a=0;a<source.length();a++)if(a!=idx)next.put(source.opt(a));
                        XsGpsLicense.prefs(activity).edit().putString("bookmarks",next.toString()).apply();
                        refreshSaved();
    ...
    private void search(){
        ...
        try{
            ...
            bg.execute(()->{
                try{
                    List<Address> results=new Geocoder(activity,Locale.getDefault()).getFromLocationName(q,1);
                ...
                }catch(Exception ex){activity.runOnUiThread(()->toast("البحث بالاسم غير متوفر على هذا الجهاز، جرّب الإحداثيات."));}
            });
    ...
    private void openMap(){
        try{
            ...
            AlertDialog dialog=new AlertDialog.Builder(activity).setTitle("حدد الموقع على الخريطة")
            ...
            web.loadUrl(String.format(Locale.US,"file:///android_asset/xsgps_map.html?lat=%.7f&lon=%.7f",p[0],p[1]));
            dialog.show();
            dialog.getWindow().setLayout(-1,(int)(activity.getResources().getDisplayMetrics().heightPixels*0.82));
        }catch(Exception e){toast("أدخل إحداثيات صحيحة أولًا");}
    }
    private void startMock(){
        if(!!XsGpsOverlay.activationRequired() && !XsGpsLicense.recentlyVerified(activity)){
        ...
        if(activity.checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)!=PackageManager.PERMISSION_GRANTED){
            activity.requestPermissions(new String[]{Manifest.permission.ACCESS_FINE_LOCATION,
                  Manifest.permission.ACCESS_COARSE_LOCATION},30115);
            toast("امنح إذن الموقع الدقيق ثم اضغط تشغيل مرة أخرى");return;
        }
        XsGpsLicense.prefs(activity).edit().putString("lat",latitude.getText().toString())
        ...
        Intent s=new Intent(activity,XsGpsLocationService.class).setAction(XsGpsLocationService.START)
            .putExtra("lat",p[0]).putExtra("lon",p[1])
            .putExtra("movement",randomMovement.isChecked())
            .putExtra("radius",(double)radiusSeek.getProgress());
        try{activity.startForegroundService(s);toast("جاري تشغيل الموقع التجريبي");state.postDelayed(this::updateStatus,900);}
        catch(Exception e){toast("فشل تشغيل الخدمة. امنح إذن الموقع واختر التطبيق للموقع التجريبي.");}
    }
    private void stopMock(){
        activity.startService(new Intent(activity,XsGpsLocationService.class).setAction(XsGpsLocationService.STOP));
        state.postDelayed(this::updateStatus,450);
        toast("تم إرسال أمر الإيقاف");
    }
    private void updateStatus(){
        android.content.SharedPreferences p=XsGpsLicense.prefs(activity);
    ...
    @Override protected void onStart(){
        ...
        if(getWindow()!=null)getWindow().setLayout(
            ...
            (int)(activity.getResources().getDisplayMetrics().heightPixels*0.91));
        updateStatus();
    }
}
