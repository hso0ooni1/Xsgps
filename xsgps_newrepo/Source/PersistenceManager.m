...
static NSString * const kSuiteName = @"com.xsgps.dylib";
...
- (void)clearSpoof {
    os_unfair_lock_lock(&_lock);
    // XsGpS keeps the last selected coordinate while turning spoofing OFF.
    // This matches the current UI behavior: the location remains selected,
    // but no location is spoofed until the user enables the main switch again.
    self.cachedEnabled = NO;
    self.cachedSimulationWasActive = NO;
    [self.defaults setBool:NO forKey:kKeyEnabled];
